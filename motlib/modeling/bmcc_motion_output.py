...
class MotionOutputs:
    ...
    def morigin_loss(self):
        ...
        if self.pred_morigin[self.gt_motion_valids].size(0) == 0:
            ...
            rot_inds = ((self.gt_types[self.gt_motion_valids] == 0).nonzero().unbind(1)[0])
    ...
    def losses(self):
        ...
        if self.only_DET:
            return {
                ...
                "loss_mtype": torch.tensor(0.0, device=self.gt_classes.device),
...
class MotionOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # CHOICES of different structures

        # MotionNet 2.0
        # self.mtype_layer = Linear(input_size, 1)
        # self.morigin_layer = Linear(input_size, 3)
        # self.maxis_layer = Linear(input_size, 3)
        # nn.init.kaiming_normal_(self.mtype_layer.weight,a=0,mode='fan_in',nonlinearity='leaky_relu')
        # nn.init.kaiming_normal_(self.morigin_layer.weight,a=0,mode='fan_in',nonlinearity='leaky_relu')
        # nn.init.kaiming_normal_(self.maxis_layer.weight,a=0,mode='fan_in',nonlinearity='leaky_relu')

        # Add prediction layer for motion type, motion origin and motion axis
        # For motion type, the output is just a number between 0 - 1
    ...
    def forward(self, x):
        ...
        return scores, proposal_deltas, mtype, morigin, maxis
    ...
    def losses(self, predictions, proposals):
        ...
        """
        scores, proposal_deltas, mtype, morigin, maxis = predictions
    ...
    def predict_boxes_for_gt_classes(self, predictions, proposals):
        ...
        if self.use_GTBBX:
            proposal_deltas = torch.zeros(proposal_deltas.size(), device=proposal_deltas.device)
...
def motion_inference(
    ...
    result_per_image = [
        ...
        for scores_per_image, boxes_per_image, mtype_per_image, morigin_per_image, maxis_per_image, image_shape in zip(
            scores, boxes, mtype, morigin, maxis, image_shapes
        )
    ]
...
def motion_inference_single_image(
    ...
    boxes, scores, mtype, morigin, maxis, filter_inds = (
        boxes[keep],
        scores[keep],
        mtype[keep],
        morigin[keep],

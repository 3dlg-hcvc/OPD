...
BEST_RGB_MODEL = "/scratch/hanxiao/proj-motionnet/train_output/done/motion_finetuning_oc_0.001_60000_0.5_15/finetune_oc_rgb_3___1_8_8/model_final.pth"
BEST_DEPTH_MODEL = "/scratch/hanxiao/proj-motionnet/train_output/done/motion_finetuning_oc_0.001_60000_0.5_15/finetune_oc_depth_2___1_8_8/model_final.pth"
BEST_RGBD_MODEL = "/scratch/hanxiao/proj-motionnet/train_output/done/motion_finetuning_oc_0.001_60000_0.5_15/finetune_oc_rgbd_2___1_8_8/model_final.pth"
EMAIL_ADDR = "shawn_jiang@sfu.ca"
OUTPUT_DIR = "/home/hanxiao/scratch/proj-motionnet/train_output"
name = f"motion_finetuning_oc_{BASE_LR}_{MAX_ITER}_{FLIP_PROB}_{EXTRINSIC_WEIGHT}_{ablation_string}"
...
for setting in settings:
    ...
    for motion_weight in MOTION_WEIGHTS:
        for i in range(1, NUM_RANDOM + 1, 1):
            ...
            script.append(
                f"python $PROJ_DIR/train.py --config-file {config} {ablation_command} --output-dir $OUTPUT_DIR/{name}/{run_name} --data-path $DATASET_DIR/MotionDataset_h5_6.11 --input-format {input_format} --model_attr_path $PROJ_DIR/scripts/data/data_statistics/urdf-attr.json --flip_prob {FLIP_PROB} --extrinsic_weight {EXTRINSIC_WEIGHT} --motion_weights {motion_weight[0]} {motion_weight[1]} {motion_weight[2]} {other_option} INPUT.RNG_SEED {i} {extra}"
            )

...
PIXEL_MEAN = "'[144.7425400388733, 131.67830996768458, 113.38040344244014, 975.0775146484375]'"
PIXEL_STD = "'[20.100716763269578, 20.805474870130748, 23.863171739073888, 291.606201171875]'"
...
BEST_RGB_MODEL = "/home/hanxiao/scratch/proj-motionnet/train_output/done/only_det_real_0.0005_30000/det_cc_rgb_1/model_0019999.pth"
BEST_DEPTH_MODEL = "/home/hanxiao/scratch/proj-motionnet/train_output/done/only_det_real_0.0005_30000/det_cc_depth_1/model_0019999.pth"
BEST_RGBD_MODEL = "/home/hanxiao/scratch/proj-motionnet/train_output/done/only_det_real_0.0005_30000/det_cc_rgbd_1/model_0019999.pth"
...
name = f"motion_finetuning_oc_real_draft_{BASE_LR}_{MAX_ITER}_{FLIP_PROB}_{EXTRINSIC_WEIGHT}_{ablation_string}"
...
settings = ['finetune_oc_rgb', 'finetune_oc_depth', 'finetune_oc_rgbd']
# settings = ["finetune_oc_rgb"]
...
for setting in settings:
    ...
    for motion_weight in MOTION_WEIGHTS:
        for i in range(1, NUM_RANDOM + 1, 1):
            ...
            script.append(
                f"python $PROJ_DIR/train.py --config-file {config} {ablation_command} --output-dir $OUTPUT_DIR/{name}/{run_name} --data-path $DATASET_DIR/MotionDataset_h5_real --input-format {input_format} --model_attr_path $PROJ_DIR/scripts/data/data_statistics/real-attr.json --flip_prob {FLIP_PROB} --extrinsic_weight {EXTRINSIC_WEIGHT} --motion_weights {motion_weight[0]} {motion_weight[1]} {motion_weight[2]} {other_option} INPUT.RNG_SEED {i} {extra} MODEL.PIXEL_MEAN {PIXEL_MEAN} MODEL.PIXEL_STD {PIXEL_STD}"
            )

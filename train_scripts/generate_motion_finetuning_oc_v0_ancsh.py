...
NUM_RANDOM = 1
BEST_RGB_MODEL = "/home/hanxiao/scratch/proj-motionnet/train_output/done/only_det_ancsh_0.0005_30000/det_cc_rgb_1/model_0019999.pth"
BEST_DEPTH_MODEL = "/home/hanxiao/scratch/proj-motionnet/train_output/done/only_det_ancsh_0.0005_30000/det_cc_depth_1/model_0014999.pth"
BEST_RGBD_MODEL = "/home/hanxiao/scratch/proj-motionnet/train_output/done/only_det_ancsh_0.0005_30000/det_cc_rgbd_1/model_0019999.pth"
EMAIL_ADDR = "shawn_jiang@sfu.ca"
OUTPUT_DIR = "/home/hanxiao/scratch/proj-motionnet/train_output"
name = f"motion_finetuning_ocv0_ancsh_{BASE_LR}_{MAX_ITER}_{SOLVER_1}_{SOLVER_2}_{FLIP_PROB}_{EXTRINSIC_WEIGHT}_{DET_EXTRINSIC_ITER}"
...
for setting in settings:
    ...
    for motion_weight in MOTION_WEIGHTS:
        for i in range(1, NUM_RANDOM + 1, 1):
            ...
            script.append(f"python $PROJ_DIR/train.py --config-file {config} --output-dir $OUTPUT_DIR/{name}/{run_name} --data-path $DATASET_DIR/MotionDataset_h5_ancsh --input-format {input_format} --model_attr_path $PROJ_DIR/scripts/data/data_statistics/urdf-attr.json --flip_prob {FLIP_PROB} --motion_weights {motion_weight[0]} {motion_weight[1]} {motion_weight[2]} {extra_v0} {other_option} INPUT.RNG_SEED {i} {extra}")

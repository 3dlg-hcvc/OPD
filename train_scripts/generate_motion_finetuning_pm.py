...
# POSE_WEIGHTS =[[1, 1, 1]]
motion_weight = [1, 8, 8]
settings = ['finetune_pm_rgb', 'finetune_pm_depth', 'finetune_pm_rgbd']
# settings = ['finetune_pm_rgb']
name = f'motion_finetuning_pm_normalize_{BASE_LR}_{MAX_ITER}_{SOLVER_1}_{SOLVER_2}_{POSE_ITER}_{MOTION_ITER}_{motion_weight[0]}_{motion_weight[1]}_{motion_weight[2]}'
...
for setting in settings:
    for pose_weight in POSE_WEIGHTS:
        for i in range(1, NUM_RANDOM+1, 1):
            run_name = f"{setting}_{i}___{pose_weight[0]}_{pose_weight[1]}_{pose_weight[2]}"
...
for setting in settings:
    ...
    for pose_weight in POSE_WEIGHTS:
        for i in range(1, NUM_RANDOM+1, 1):
            ...
            script.append(f"python $PROJ_DIR/train.py --config-file {config} --output-dir $OUTPUT_DIR/{name}/{run_name} --data-path $DATASET_DIR/MotionDataset_h5_6.11 --input-format {input_format} --model_attr_path $PROJ_DIR/scripts/data/data_statistics/urdf-attr.json --motion_weights {motion_weight[0]} {motion_weight[1]} {motion_weight[2]} --pose_weights {pose_weight[0]} {pose_weight[1]} {pose_weight[2]} {other_option} INPUT.RNG_SEED {i} {extra}")

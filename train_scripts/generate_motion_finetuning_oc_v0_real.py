...
name = f"motion_finetuning_ocv0_real_draft_{BASE_LR}_{MAX_ITER}_{SOLVER_1}_{SOLVER_2}_{FLIP_PROB}_{EXTRINSIC_WEIGHT}_{DET_EXTRINSIC_ITER}"
...
for setting in settings:
    ...
    for motion_weight in MOTION_WEIGHTS:
        for i in range(1, NUM_RANDOM + 1, 1):
            ...
            script.append(f"python $PROJ_DIR/train.py --config-file {config} --output-dir $OUTPUT_DIR/{name}/{run_name} --data-path $DATASET_DIR/MotionDataset_h5_real --input-format {input_format} --model_attr_path $PROJ_DIR/scripts/data/data_statistics/real-attr.json --flip_prob {FLIP_PROB} --motion_weights {motion_weight[0]} {motion_weight[1]} {motion_weight[2]} {extra_v0} {other_option} INPUT.RNG_SEED {i} {extra} MODEL.PIXEL_MEAN {PIXEL_MEAN} MODEL.PIXEL_STD {PIXEL_STD}")

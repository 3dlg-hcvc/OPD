...
name = f'only_det_real_{BASE_LR}_{MAX_ITER}'
...
for setting in settings:
    ...
    for i in range(1, NUM_RANDOM+1, 1):
        ...
        script.append(f"python $PROJ_DIR/train.py --config-file {config} --output-dir $OUTPUT_DIR/{name}/{setting}_{i} --data-path $DATASET_DIR/MotionDataset_h5_real --input-format {input_format} --model_attr_path $PROJ_DIR/scripts/data/data_statistics/real-attr.json {other_option} INPUT.RNG_SEED {i} MODEL.PIXEL_MEAN {PIXEL_MEAN} MODEL.PIXEL_STD {PIXEL_STD}")

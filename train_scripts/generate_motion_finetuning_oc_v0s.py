...
FLIP_PROB = 0.5
# The last one is the motion state weight
MOTION_WEIGHTS = [[1, 8, 8, 1], [1, 8, 8, 8], [1, 8, 8, 15]]
NUM_RANDOM = 1
...
name = f"motion_finetuning_ocv0_state_bgfg_{BASE_LR}_{MAX_ITER}_{SOLVER_1}_{SOLVER_2}_{FLIP_PROB}_{EXTRINSIC_WEIGHT}_{DET_EXTRINSIC_ITER}_{ablation_string}"
settings = ['finetune_ocv0s_rgb', 'finetune_ocv0s_depth', 'finetune_ocv0s_rgbd']
...
for setting in settings:
    for motion_weight in MOTION_WEIGHTS:
        for i in range(1, NUM_RANDOM + 1, 1):
            run_name = f"{setting}_{i}___{motion_weight[0]}_{motion_weight[1]}_{motion_weight[2]}_{motion_weight[3]}"
...
for setting in settings:
    ...
    elif model_name == "ocv0s":
        config = "$PROJ_DIR/configs/bmoc_v0s.yaml"
        extra_v0 = f"--image_state_path $PROJ_DIR/scripts/data/motion_state/image_close.json --extrinsic_weight {EXTRINSIC_WEIGHT} --det_extrinsic_iter {DET_EXTRINSIC_ITER}"
    ...
    for motion_weight in MOTION_WEIGHTS:
        for i in range(1, NUM_RANDOM + 1, 1):
            ...
            script.append(f"python $PROJ_DIR/train.py --config-file {config} {ablation_command} --output-dir $OUTPUT_DIR/{name}/{run_name} --data-path $DATASET_DIR/MotionDataset_h5_6.11 --input-format {input_format} --model_attr_path $PROJ_DIR/scripts/data/data_statistics/urdf-attr.json --flip_prob {FLIP_PROB} --motion_weights {motion_weight[0]} {motion_weight[1]} {motion_weight[2]} --mstate_weight {motion_weight[3]} {extra_v0} {other_option} INPUT.RNG_SEED {i} {extra}")
